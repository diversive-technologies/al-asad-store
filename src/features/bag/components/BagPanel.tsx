...
export function BagPanel({ locale, messages }: BagPanelProps) {
  ...
  const { isOpen, close, bag, onSummary } = useBag();
  ...
  return (
    ...
    >
      ...
      <p aria-live="polite" role={notice === null ? undefined : 'alert'} className="text-danger-500 text-sm empty:hidden">
        {notice}
      </p>
      ...
      {summary === undefined || summary.lines.length === 0 ? null : (
        <div className="text-fg-muted mt-4 flex items-center gap-1.5 text-xs">
          ...
          {/* The popover renders a heading and paragraphs, so its trigger and
              content cannot sit inside a <p> — the parser would close it. */}
          <InfoPopover
