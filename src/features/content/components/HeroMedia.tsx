...
export function HeroMedia({ poster, video }: HeroMediaProps) {
  ...
  /*
   * The film COVERS the section - full width and full height, cropped rather
   * than letterboxed - so it reaches both side edges at every screen size, and
   * `hero-frame` alone decides how tall that is: never more than one screen.
   *
   * It used to be a 16:9 stage sized from the section's HEIGHT. That left bands
   * beside the film on any viewport wider than 16:9, and most desktop browser
   * viewports are, once the browser's own chrome comes off the screen. A glow
   * sampled from the playing frame filled those bands and spilled below the
   * film. Both are gone: a film that covers has no bands to fill.
   *
   ...
   * caption and its scrim are anyway.
   */
  return (
