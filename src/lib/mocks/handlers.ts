...
export const handlers = [
  ...
  /*
   * Architecture §24 Try-On — module 14.
   *
   ...
   */
  http.get(`*${ENDPOINTS.tryOn.offer}`, () =>
    HttpResponse.json(
      tryOnOffer({ sampleWhenUnconfigured: serverEnv.TRY_ON_SAMPLE_RESULT === 'enabled' }),
    ),
  ),
  ...
  http.post(`*${ENDPOINTS.tryOn.generate}`, async ({ request }) => {
    ...
    const outcome = await generateTryOn(
      ...
      // Standing in for Java configuration: the module is told the policy.
      { sampleWhenUnconfigured: serverEnv.TRY_ON_SAMPLE_RESULT === 'enabled' },
    );

    /*
     * PHOTO_REJECTED is deliberately NOT a §24 outcome, so it does not travel on
     * the success union — it becomes a 400 here, which `apiRequest` normalises
     * to a VALIDATION error. Smuggling it through as a third success member
     * would have made every caller handle a failure while holding a value the
     * type says succeeded.
     */
    if (outcome.status === 'PHOTO_REJECTED') return new HttpResponse(null, { status: 400 });
